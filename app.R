...
server <- function(input, output,session) {
  ...
  hospitals <- reactive({
    ...
    hospitals <- read.csv(here("www/healthmopupandbaselinenmisfacility.csv"))
    return(hospitals)
  })
  
  # Define hospitals1 as a reactive expression
  hospitals1 <- reactive({
    hospitals <- hospitals()  # Ensure hospitals() has data
    hospitals1 <- hospitals %>%
      drop_na(c("longitude", "latitude"))  # Quoting column names
    return(hospitals1)
  })
  ...
  output$map1 <- renderLeaflet({
    ...
    req(hospitals1())  # Ensure hospitals1() is available
    data <- hospitals1()  # Get the data from hospitals1
    ...
    data$icon <- case_when(
      data$facility_type_display == "Health Post" ~ "HealthPost",
      data$facility_type_display == "Basic Health Centre / Primary Health Clinic" ~ "BasicHealthCentre",
      data$facility_type_display == "Primary Health Centre (PHC)" ~ "PrimaryHealthCentre",
      data$facility_type_display == "Dispensary" ~ "Dispensary",
      data$facility_type_display == "General Hospital" ~ "GeneralHospital",
      data$facility_type_display == "Information not available / Don't know" ~ "Informationnotavailable",
      data$facility_type_display == "Basic Health Centre or Primary Health Clinic" ~ "BasicHealthCentre",
      data$facility_type_display == "District Hospital / Comprehensive Health Centre" ~ "ComprehensiveHealthCentre",
      data$facility_type_display == "Teaching Hospital" ~ "TeachingHospital",
      data$facility_type_display == "District Hospital or Comprehensive Health Centre" ~ "ComprehensiveHealthCentre",
      data$facility_type_display == "Primary Health Center" ~ "PrimaryHealthCentre",
      data$facility_type_display == "District / General Hospital" ~ "DistrictGeneralHospital",
      data$facility_type_display == "Clinic" ~ "Clinic",
      data$facility_type_display == "Teaching / Specialist Hospital" ~ "TeachingHospital",
      data$facility_type_display == "Private Facility" ~ "PrivateFacility",
      data$facility_type_display == "Specialist Hospital" ~ "SpecialistHospital",
    ...
    leaflet(data) %>%
      ...
      setView(lng = mean(data$longitude, na.rm = TRUE), lat = mean(data$latitude, na.rm = TRUE), zoom = 10) %>%
      addAwesomeMarkers(
        lng = data$longitude, 
        lat = data$latitude,
        icon = ~iconSet[data$icon],  # Use the icon set based on the new icon column
        popup = data$facility_name,
